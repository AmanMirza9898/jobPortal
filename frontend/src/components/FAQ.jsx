            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div className="space-y-4">
                    {FAQData.map((faq, index) => (
                        <div key={index} className="border border-gray-200 dark:border-white/10 rounded-lg overflow-hidden bg-white/50 dark:bg-white/5 backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow duration-300">
                            <button
                                className="w-full px-6 py-4 text-left flex justify-between items-center focus:outline-none bg-transparent hover:bg-white/30 dark:hover:bg-white/10 transition-colors duration-200"
                                onClick={() => toggleFAQ(index)}
                            >
                                <span className="text-lg font-medium text-gray-800 dark:text-gray-200">{faq.question}</span>
                                <span className={`ml-6 shrink-0 text-gray-500 dark:text-gray-400 transform transition-transform duration-300 ${openIndex === index ? 'rotate-45' : ''}`}>
                            ...
                            >
                                <p className="px-6 py-4 text-gray-600 dark:text-gray-300 border-t border-gray-100 dark:border-white/10 leading-relaxed">
                                    {faq.answer}
                                </p>
                            </div>
                        </div>
